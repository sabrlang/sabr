...
#define list_node_at(TYPE, p_l, index) list_func(at, TYPE)(p_l, index)
...
#define list_at(TYPE, p_l, index) (&(list_func(node_at, TYPE)(p_l, index)->data))
...
#define list_imp_c(TYPE) \
	...
	node(TYPE)* list_func(node_at, TYPE)(list(TYPE)* p_l, size_t index) { \
		...
		if (index == 0 || index == (p_l->size -1)) return temp_node; \
